...
export function equationsDisplayed(vis: d3.Selection<SVGSVGElement, unknown, null, undefined>, numPhases: number): void {
    let voltageEquation: string;
    let aEquation: string;
    let a2Equation: string;
...
`;
    ...
    const voltageNode = voltageDiv.node() as HTMLElement | null;
    const aNode = aDiv.node() as HTMLElement | null;
    const a2Node = a2Div.node() as HTMLElement | null;

    if (voltageNode) {
        katex.render(voltageEquation, voltageNode, { throwOnError: false });
    }
    if (aNode) {
        katex.render(aEquation, aNode, { throwOnError: false });
    }
    if (a2Node) {
        katex.render(a2Equation, a2Node, { throwOnError: false });
    }
}
